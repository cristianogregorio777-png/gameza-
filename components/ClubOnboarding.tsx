...
export default function ClubOnboarding({ onBack, onCustomize, onSkip }: ClubOnboardingProps) {
  return (
    <section className="min-h-screen px-4 pb-28 pt-6 lg:flex lg:min-h-[calc(100vh-2rem)] lg:items-center lg:justify-center">
      <div className="w-full max-w-[480px] rounded-[30px] border border-cream/15 bg-navy-deep p-5 lg:p-8">
        ...
        <div className="mt-6 rounded-[22px] border border-cream/15 bg-navy-soft p-4">
          <div className="mb-4 inline-flex rounded-full border border-orange/40 bg-orange/15 px-2.5 py-1 text-[9px] font-semibold text-orange">
            Clube
          </div>
        ...
        <div className="mt-6 space-y-3">
          <button
            onClick={onCustomize}
            className="flex w-full items-center justify-center gap-2 rounded-pill bg-orange py-3.5 text-sm font-bold text-cream"
          >
            Personalizar clube
            <ArrowRight size={16} />
          </button>
          ...
          <button
            onClick={onSkip}
            className="flex w-full items-center justify-center gap-2 rounded-pill border border-cream/20 bg-navy-soft py-3.5 text-sm font-semibold text-cream"
          >
            <Plus size={16} />
            Pular por agora
          </button>
...
function FeatureRow({ icon, title, description }: { icon: React.ReactNode; title: string; description: string }) {
  return (
    <div className="flex gap-3 rounded-2xl border border-cream/15 bg-navy-soft p-3.5">
      <div className="mt-0.5 flex h-9 w-9 items-center justify-center rounded-xl bg-olive text-cream">{icon}</div>
      <div>
